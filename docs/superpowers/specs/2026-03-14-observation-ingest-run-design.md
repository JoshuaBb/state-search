...
- **Update** `bulk_create` to accept `&PgPool` directly and include `ingest_run_id` and `inserted_at` in the INSERT columns.
...
- At the start of `run()`, generate `let ingest_run_id = Uuid::new_v4()`.
...
- Dimension upserts (`dim_location`, `dim_time`) and raw import inserts remain pool-level (unchanged — they were already outside the transaction scope).
